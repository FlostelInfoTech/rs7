...
struct TerserPath {
    ...
    subcomponent_index: Option<usize>,
}
...
impl TerserPath {
    ...
    fn parse_segment_part(part: &str) -> Result<(String, usize)> {
        if let Some(paren_pos) = part.find('(') {
            ...
            let close_paren = index_str.find(')').ok_or_else(|| {
                Error::terser_path("Missing closing parenthesis")
            })?;
            let index = index_str[..close_paren].parse::<usize>().map_err(|_| {
                Error::terser_path("Invalid segment index")
            })?;
            Ok((id, index))
        } else {
            Ok((part.to_string(), 0))
    ...
    fn parse_field_part(part: &str) -> Result<(usize, usize)> {
        if let Some(paren_pos) = part.find('(') {
            ...
            let field_index = field_str.parse::<usize>().map_err(|_| {
                Error::terser_path("Invalid field index")
            })?;

            let rep_str = &part[paren_pos + 1..];
            let close_paren = rep_str.find(')').ok_or_else(|| {
                Error::terser_path("Missing closing parenthesis")
            })?;
            let rep_index = rep_str[..close_paren].parse::<usize>().map_err(|_| {
                Error::terser_path("Invalid repetition index")
            })?;
    ...
    /// Parse a simple index
    fn parse_index(part: &str) -> Result<usize> {
        part.parse::<usize>().map_err(|_| {
            Error::terser_path(format!("Invalid index: {}", part))
        })
    }
}
